...
export function createSearchJournalEntriesTool(familyId: number) {
  return tool({
    description:
      "Search the family's journal entries by keyword and/or date range. Use this to answer questions " +
    ...
    execute: async ({ keyword, fromDate, toDate }) => {
      const conditions = [eq(journalEntries.familyId, familyId)];
      if (keyword) {
        conditions.push(
...
export function createGetMilestoneEntriesTool(familyId: number) {
  return tool({
    description:
      "Get all journal entries tagged with a specific milestone category (food, social, physical, language, " +
      "health, place, special_day, other). Use this for 'when did they first ___' style questions.",
    inputSchema: z.object({
    ...
    execute: async ({ milestoneCategory }) => {
      return db
        .select()
        .from(journalEntries)
        .where(and(eq(journalEntries.familyId, familyId), eq(journalEntries.milestoneCategory, milestoneCategory)))
        .orderBy(journalEntries.entryDate);
    },
  });
}
