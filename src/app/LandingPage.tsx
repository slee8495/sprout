  ...
  return (
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-10 p-6 pb-16">
      <section className="flex flex-col items-center gap-4 pt-12 text-center">
        <h1 className="font-heading text-4xl font-bold text-brand-700 dark:text-brand-300">🌱 Roun</h1>
        <p className="max-w-sm text-lg text-zinc-600 dark:text-zinc-400">
          <T>A private, lifelong journal for your family.</T>
        </p>
        <Link
          href="/login"
          className="mt-2 rounded-full bg-brand-600 px-6 py-3 font-heading font-semibold text-white shadow-sm shadow-brand-900/20 transition-transform hover:scale-105 hover:bg-brand-700 active:scale-95"
        >
          <T>Get started</T>
        </Link>
      </section>
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {FEATURES.map((feature) => (
          <div
            key={feature.title}
            className="flex flex-col gap-2 rounded-3xl border border-brand-200/70 bg-white p-5 shadow-md shadow-brand-900/5 dark:border-brand-800/50 dark:bg-zinc-900 dark:shadow-black/40"
          >
