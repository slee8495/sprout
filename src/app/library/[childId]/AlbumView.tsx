    ...
    <div className={`${albumSerif.variable} mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 p-4 pb-24`}>
      ...
      {pages.length === 0 ? (
        ...
        <div className="flex flex-col gap-8">
          {pages.map((page, i) => (
            <AlbumPageFrame key={i} page={page} coverBackground={child.coverBackground} coverAnimal={child.coverAnimal} />
          ))}
        </div>
      ) : (
        <div className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-4">
          {pages.map((page, i) => (
            <div key={i} className="w-full shrink-0 snap-start">
              <AlbumPageFrame page={page} coverBackground={child.coverBackground} coverAnimal={child.coverAnimal} />
            </div>
          ))}
        </div>
      )}
...
function AlbumPageFrame({
  ...
  return (
    <div className="aspect-[4/3] overflow-hidden rounded-2xl border border-brand-100/60 bg-[#fffaf0] p-3 shadow-md shadow-brand-900/5 dark:border-brand-900/40 dark:bg-zinc-900 dark:shadow-black/40">
      {page.kind === "month" ? (
        <MonthDividerPage date={page.dates[0]} coverBackground={coverBackground} coverAnimal={coverAnimal} />
      ) : (
